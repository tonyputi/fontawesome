"""Convert pinned Lucide stroke SVGs to mono-vertical bitmaps.

The converter uses only the Python standard library and runs fully offline on
the vendored SVGs in ``assets/lucide/icons``. Lucide icons live on a 24x24
viewBox and are drawn as round-capped strokes, so every shape is flattened to
polylines and rasterized with fixed supersampling. The output byte layout
matches the Font Awesome catalog: column-major pages with the top pixel in the
least significant bit (``uicons::PixelFormat::MonoVertical``).
"""
...
def rasterize_svg(text: str, size: int) -> Tuple[int, ...]:
    """Rasterize one Lucide SVG to mono-vertical bytes at ``size`` pixels."""
