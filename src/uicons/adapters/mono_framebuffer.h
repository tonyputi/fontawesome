...
struct MonoFramebuffer {
    ...
    constexpr MonoFramebuffer(uint8_t* data_, uint16_t width_, uint16_t height_, uint16_t stride_)
        : data(data_), width(width_), height(height_), stride(stride_) {}
};

namespace detail {

inline void setFramebufferPixel(void* context, int16_t x, int16_t y, uint32_t color) {
    MonoFramebuffer& framebuffer = *static_cast<MonoFramebuffer*>(context);
    const uint32_t index =
        static_cast<uint32_t>(y) * framebuffer.stride + static_cast<uint16_t>(x) / 8u;
...
inline bool render(const Icon& icon, MonoFramebuffer& framebuffer, int16_t x, int16_t y,
                   uint32_t color = 1) {
    ...
    const Canvas canvas(&framebuffer, framebuffer.width, framebuffer.height,
