...
struct Canvas {
    ...
    constexpr Canvas(void* context_,
                     uint16_t width_,
                     uint16_t height_,
...
inline bool validIcon(const Icon& icon) {
    ...
    switch (icon.format) {
        ...
        case PixelFormat::MonoVertical:
            ...
            expectedBytes = static_cast<uint32_t>(icon.stride) *
                            ((static_cast<uint32_t>(icon.height) + 7u) / 8u);
            break;
...
inline bool render(const Icon& icon,
                  ...
                  int16_t y,
                  uint32_t color = 1) {
    if (canvas.context == nullptr || canvas.setPixel == nullptr ||
        canvas.width == 0 || canvas.height == 0 ||
        !detail::validIcon(icon)) {
        return false;
    }
    ...
    for (uint16_t iconY = 0; iconY < icon.height; ++iconY) {
        for (uint16_t iconX = 0; iconX < icon.width; ++iconX) {
            ...
            if (detail::inside(canvas, targetX, targetY)) {
                canvas.setPixel(canvas.context,
